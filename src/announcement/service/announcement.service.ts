...
export class AnnouncementService {
    ...
    async update(id: string, dto: UpdateAnnouncementDto, actorId: string): Promise<Announcement> {
        const announcement = await this.getOrThrow(id);

        if (dto.title !== undefined) announcement.title = dto.title;
        if (dto.body !== undefined) announcement.body = this.sanitizationService.sanitizeForEmail(dto.body);
        ...
        this.auditLogService.log('ANNOUNCEMENT_UPDATED', {
            actorId,
            targetId: id,
            metadata: {title: saved.title, audience: saved.audience, changes: Object.keys(dto)},
        });
        return saved;
    }
