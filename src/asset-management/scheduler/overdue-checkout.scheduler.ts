...
export class OverdueCheckoutScheduler {
    ...
    async dispatchOverdueReminders(): Promise<void> {
        ...
        const acquired = await this.cacheService.acquireLock(OverdueCheckoutScheduler.LOCK_KEY, 300);
        if (!acquired) return;
    ...
    private parseThresholds(): number[] {
        const raw = this.configService.get<string>('ASSET_OVERDUE_NOTIFICATION_DAYS', '1,3,7');
        if (!raw.trim()) return [];
        return raw
            .split(',')
    ...
    private async runReminders(thresholds: number[]): Promise<void> {
        ...
        for (const checkout of overdueCheckouts) {
            try {
                ...
                this.logger.error(`Failed to process overdue reminder for checkout ${checkout.id}`, err);
            }
        }
    }
    ...
    private async fetchAdminEmails(): Promise<string[]> {
        ...
        return admins
            .filter(a => a.adminRole?.permissions?.includes(AdminPermission.ASSET_MANAGEMENT_READ))
    ...
    private async processCheckout(
        ...
        const daysOverdue = Math.floor(
            (now.getTime() - (checkout.expectedReturnAt?.getTime() ?? now.getTime())) / 86_400_000,
        );

        const alreadySent = await this.notificationRepo.find({
            where: {checkout: {id: checkout.id}, type: CheckoutNotificationType.OVERDUE_REMINDER},
            select: ['daysOverdue'],
        });
    ...
    private async sendOverdueNotification(
        ...
        const templateData = {
            ...
            expectedReturnAt: checkout.expectedReturnAt?.toDateString() ?? 'Not specified',
            daysOverdue,
        ...
        if (checkout.checkedOutToMember?.id) {
            ...
            if (member?.email) {
                this.utilityService.sendEmailWithTemplate(member.email, subject, 'asset-overdue-reminder', {
                    ...templateData,
        ...
        if (checkout.checkedOutToDepartment?.id) {
            ...
            for (const lead of leads) {
                ...
                if (email) {
                    this.utilityService.sendEmailWithTemplate(email, subject, 'asset-overdue-reminder', {
                        ...templateData,
                        recipientName: name,
                    });
                }
        ...
        for (const email of adminEmails) {
            this.utilityService.sendEmailWithTemplate(email, `[Admin] ${subject}`, 'asset-overdue-reminder', {
                ...templateData,
                recipientName: 'Admin',
            });
        }
