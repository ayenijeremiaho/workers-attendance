...
export class WarrantyAlertScheduler {
    ...
    async dispatchWarrantyAlerts(): Promise<void> {
        const acquired = await this.cacheService.acquireLock(WarrantyAlertScheduler.LOCK_KEY, 300);
        if (!acquired) return;
    ...
    private async runAlerts(): Promise<void> {
        ...
        for (const asset of assets) {
            try {
                ...
                this.logger.error(`Failed to process warranty alert for asset ${asset.id}`, err);
            }
        }
    }
