...
import {AssetCheckoutNotification, CheckoutNotificationType} from '../entity/asset-checkout-notification.entity';
...
export class AssetCheckoutService {
    ...
    async create(assetId: string, dto: CreateCheckoutDto, admin: Admin): Promise<AssetCheckout> {
        if (!dto.checkedOutToMemberId && !dto.checkedOutToDepartmentId) {
            throw new BadRequestException('Either checkedOutToMemberId or checkedOutToDepartmentId must be provided.');
        }
        ...
        if (asset.status === AssetStatus.UNDER_MAINTENANCE) {
            throw new BadRequestException('Asset is currently under maintenance and cannot be checked out.');
        }
        if (asset.status === AssetStatus.DECOMMISSIONED) {
            throw new BadRequestException('Asset has been decommissioned and cannot be checked out.');
        }
        if (asset.status === AssetStatus.INACTIVE) {
            throw new BadRequestException('Asset is inactive and cannot be checked out.');
        }
        ...
        if (activeCheckout) {
            throw new BadRequestException('This asset already has an active checkout. Return it before checking out again.');
        }

        const checkout = this.checkoutRepo.create({
            asset: {id: assetId} as any,
            checkedOutToMember: dto.checkedOutToMemberId ? ({id: dto.checkedOutToMemberId} as any) : null,
            checkedOutToDepartment: dto.checkedOutToDepartmentId ? ({id: dto.checkedOutToDepartmentId} as any) : null,
        ...
        this.sendCheckoutNotification(asset, saved, dto, admin).catch(err =>
            this.logger.error(`Failed to send checkout notification for checkout ${saved.id}`, err),
        );

        return saved;
    }

    async returnAsset(assetId: string, checkoutId: string, dto: ReturnAssetDto, admin: Admin): Promise<AssetCheckout> {
        const checkout = await this.checkoutRepo.findOne({
            where: {id: checkoutId, asset: {id: assetId}},
            relations: ['asset', 'checkedOutBy', 'checkedOutToMember', 'checkedOutToDepartment'],
        });

        if (!checkout) throw new NotFoundException('Checkout record not found');
        if (checkout.returnedAt) throw new BadRequestException('This asset has already been returned.');
        ...
        this.sendReturnNotification(checkout, admin).catch(err =>
            this.logger.error(`Failed to send return notification for checkout ${checkoutId}`, err),
        );

        return saved;
    }

    async getHistory(assetId: string, page: number, limit: number): Promise<PaginationResponseDto<AssetCheckout>> {
        ...
        const [records, total] = await this.checkoutRepo.findAndCount({
            ...
            relations: ['checkedOutToMember', 'checkedOutToDepartment', 'checkedOutBy', 'checkedOutBy.member', 'returnedBy', 'returnedBy.member'],
    ...
    async getActiveCheckouts(page: number, limit: number): Promise<PaginationResponseDto<AssetCheckout>> {
        const [records, total] = await this.checkoutRepo.findAndCount({
            where: {returnedAt: IsNull()},
            relations: ['asset', 'checkedOutToMember', 'checkedOutToDepartment', 'checkedOutBy', 'checkedOutBy.member'],
            order: {checkedOutAt: 'DESC'},
            skip: (page - 1) * limit,
            take: limit,
        });
    ...
    private async sendCheckoutNotification(
        ...
        if (dto.checkedOutToMemberId) {
            const member = await this.memberRepo.findOne({where: {id: dto.checkedOutToMemberId}});
            if (member?.email) {
    ...
    private async sendReturnNotification(checkout: AssetCheckout, returnedByAdmin: Admin): Promise<void> {
        ...
        const templateData = {
            ...
            location: asset.location ?? 'Not specified',
            checkedOutAt: checkout.checkedOutAt.toDateString(),
            returnedAt: checkout.returnedAt?.toDateString() ?? new Date().toDateString(),
            returnedBy,
        };

        if (checkout.checkedOutToMember?.id) {
            const member = await this.memberRepo.findOne({where: {id: checkout.checkedOutToMember.id}});
            if (member?.email) {
                this.utilityService.sendEmailWithTemplate(
                    member.email,
