...
import {AssetStatus, MaintenanceCompletionStatus, MaintenanceFrequencyUnit, MaintenanceRecordType, AssetCondition} from '../enum/asset.enum';
...
describe('AssetService', () => {
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                {provide: getRepositoryToken(MaintenanceSchedule), useValue: mockScheduleRepo},
                {provide: getRepositoryToken(MaintenanceRecord), useValue: mockRecordRepo},
    ...
    describe('create', () => {
        it('auto-generates tag number when not provided', async () => {
            ...
            const created = {id: 'uuid-1', tagNumber: `AST-${new Date().getFullYear()}-0005`};
            mockAssetRepo.create.mockReturnValue(created);
            mockAssetRepo.save.mockResolvedValue(created);

            const result = await service.create({name: 'Generator', category: 'Equipment'}, mockAdmin);
        ...
        it('uses provided tag number', async () => {
            ...
            await service.create({tagNumber: 'AST-CUSTOM-001', name: 'Generator', category: 'Equipment'}, mockAdmin);
        ...
        it('throws ConflictException when tag number already exists', async () => {
            ...
            await expect(service.create({tagNumber: 'AST-2026-0001', name: 'Generator', category: 'Equipment'}, mockAdmin))
    ...
    describe('update', () => {
        it('updates only the provided fields', async () => {
            const asset = {id: 'uuid-1', name: 'Old Name', category: 'Equipment', status: AssetStatus.ACTIVE, maintenanceSchedule: null, maintenanceRecords: []};
            mockAssetRepo.findOne.mockResolvedValue(asset);
    ...
    describe('setMaintenanceSchedule', () => {
        it('creates schedule and enables maintenance when no prior schedule exists', async () => {
            const asset = {id: 'uuid-1', maintenanceSchedule: null, maintenanceRecords: []};
            mockAssetRepo.findOne.mockResolvedValueOnce(asset).mockResolvedValueOnce({...asset, maintenanceEnabled: true});
            const schedule = {};
            mockScheduleRepo.create.mockReturnValue(schedule);
            mockScheduleRepo.save.mockResolvedValue(schedule);
            mockAssetRepo.save.mockResolvedValue({...asset, maintenanceEnabled: true});

            await service.setMaintenanceSchedule('uuid-1', {
                frequencyUnit: MaintenanceFrequencyUnit.MONTHS,
                frequencyValue: 3,
                nextDueAt: '2026-09-01',
            }, mockAdmin);

            expect(mockScheduleRepo.create).toHaveBeenCalledWith({asset: {id: 'uuid-1'}});
        ...
        it('resets notification timestamps when schedule is updated', async () => {
            ...
            const asset = {id: 'uuid-1', maintenanceSchedule: existingSchedule, maintenanceRecords: []};
            mockAssetRepo.findOne.mockResolvedValue(asset);
    ...
    describe('logMaintenanceRecord', () => {
        ...
        it('sets asset status to ACTIVE when completionStatus is COMPLETED', async () => {
            const asset = {id: 'uuid-1', status: AssetStatus.UNDER_MAINTENANCE, maintenanceSchedule: null, maintenanceRecords: []};
            mockAssetRepo.findOne.mockResolvedValue(asset);
            const record = {};
            ...
            await service.logMaintenanceRecord('uuid-1', {
        ...
        it('sets asset status to UNDER_MAINTENANCE when completionStatus is IN_PROGRESS', async () => {
            const asset = {id: 'uuid-1', status: AssetStatus.ACTIVE, maintenanceSchedule: null, maintenanceRecords: []};
            mockAssetRepo.findOne.mockResolvedValue(asset);
            const record = {};
            mockRecordRepo.create.mockReturnValue(record);
            mockRecordRepo.save.mockResolvedValue(record);
        ...
        it('resets schedule and recalculates nextDueAt when COMPLETED with a schedule', async () => {
            ...
            const asset = {id: 'uuid-1', status: AssetStatus.UNDER_MAINTENANCE, maintenanceSchedule: schedule, maintenanceRecords: []};
            mockAssetRepo.findOne.mockResolvedValue(asset);
            const record = {};
            mockRecordRepo.create.mockReturnValue(record);
            mockRecordRepo.save.mockResolvedValue(record);
