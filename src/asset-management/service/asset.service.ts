...
import {AssetQueryDto, CreateAssetDto, LogMaintenanceRecordDto, SetMaintenanceScheduleDto, UpdateAssetDto, UpdateInventoryDto} from '../dto/asset.dto';
...
export class AssetService {
    ...
    async create(dto: CreateAssetDto, admin: Admin): Promise<Asset> {
        ...
        if (existing) throw new ConflictException(`Tag number '${tagNumber}' is already in use.`);
        ...
        try {
            ...
            this.auditLogService.log('ASSET_CREATED', {
                ...
                targetId: saved.id,
                metadata: {tagNumber: saved.tagNumber, name: saved.name, category: saved.category},
            });
            return saved;
        } catch (err: any) {
            if (err?.code === '23505') {
                throw new ConflictException(`Tag number '${tagNumber}' is already in use.`);
            }
            throw err;
        }
    }
    ...
    async findAll(query: AssetQueryDto): Promise<PaginationResponseDto<Asset>> {
        const {page = 1, limit = 20, status, category, maintenanceEnabled, departmentId} = query;
        ...
        if (category) qb.andWhere('LOWER(a.category) = LOWER(:category)', {category});
        if (maintenanceEnabled !== undefined) qb.andWhere('a.maintenanceEnabled = :maintenanceEnabled', {maintenanceEnabled});
    ...
    async update(id: string, dto: UpdateAssetDto, admin: Admin): Promise<Asset> {
        ...
        Object.assign(asset, {
            ...
            ...(dto.vendorContact !== undefined && {vendorContact: dto.vendorContact}),
            ...(dto.departmentId !== undefined && {department: dto.departmentId ? ({id: dto.departmentId} as any) : null}),
        });
        const saved = await this.assetRepo.save(asset);
    ...
    async setMaintenanceSchedule(id: string, dto: SetMaintenanceScheduleDto, admin: Admin): Promise<Asset> {
        const asset = await this.findOne(id);
        ...
        this.auditLogService.log('ASSET_MAINTENANCE_SCHEDULED', {
            actorId: admin.member?.id,
            targetId: id,
            metadata: {frequencyUnit: dto.frequencyUnit, frequencyValue: dto.frequencyValue, nextDueAt: dto.nextDueAt},
        });

        return this.findOne(id);
    }

    async logMaintenanceRecord(id: string, dto: LogMaintenanceRecordDto, admin: Admin): Promise<MaintenanceRecord> {
        const asset = await this.findOne(id);
        ...
        if (dto.completionStatus === MaintenanceCompletionStatus.COMPLETED) {
            ...
            if (asset.maintenanceSchedule) {
                await this.resetScheduleAfterCompletion(asset.maintenanceSchedule, dto.performedAt);
            }
        } else {
        ...
        this.auditLogService.log('ASSET_MAINTENANCE_LOGGED', {
            actorId: admin.member?.id,
            targetId: id,
            metadata: {recordId: saved.id, type: dto.type, completionStatus: dto.completionStatus},
        });

        return saved;
    }

    async updateInventory(id: string, dto: UpdateInventoryDto, admin: Admin): Promise<Asset> {
        const asset = await this.findOne(id);
    ...
    async getMaintenanceHistory(id: string, page: number, limit: number): Promise<PaginationResponseDto<MaintenanceRecord>> {
    ...
    private async resetScheduleAfterCompletion(schedule: MaintenanceSchedule, performedAt: string): Promise<void> {
