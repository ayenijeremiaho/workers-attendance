...
import {RequestDeviceResetDto, VerifyDeviceResetDto} from '../dto/device-reset.dto';
...
export class AuthController {
    ...
    @Post('login')
    async login(@Request() req: any, @Body('deviceId') deviceId: string): Promise<JwtResponse> {
    ...
    async requestDeviceReset(@Body() dto: RequestDeviceResetDto): Promise<{ message: string }> {
        await this.authService.requestDeviceReset(dto.email, dto.newDeviceId);
        return {message: 'If an account exists for this email, a device reset code has been sent.'};
    }
    ...
    async verifyDeviceReset(@Body() dto: VerifyDeviceResetDto): Promise<{ message: string }> {
        await this.authService.verifyDeviceReset(dto.email, dto.otp);
        return {message: 'Device updated successfully. You can now log in from your new device.'};
    }
