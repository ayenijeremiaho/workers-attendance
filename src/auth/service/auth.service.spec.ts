...
const mockMemberService = {
  findByEmail: jest.fn(),
  getById: jest.fn().mockResolvedValue({ id: 'member-1', firstname: 'Test', email: 'test@test.com' }),
  signup: jest.fn(),
...
describe('AuthService', () => {
  ...
  describe('logout', () => {
    it('should call sessionService.updateLogout with memberId', async () => {
      mockSessionService.updateLogout.mockResolvedValue(undefined);

      await service.logout('member-1');

      expect(mockSessionService.updateLogout).toHaveBeenCalledWith('member-1');
    });

    it('should complete without error when session exists', async () => {
      mockSessionService.updateLogout.mockResolvedValue(undefined);

      await expect(service.logout('member-1')).resolves.toBeUndefined();
    });

    it('should complete without error even when no session exists', async () => {
      mockSessionService.updateLogout.mockResolvedValue(undefined);

      await expect(service.logout('no-session-member')).resolves.toBeUndefined();
    });
  });
  ...
  describe('validateRefreshToken', () => {
    it('should throw UnauthorizedException if no session found', async () => {
      ...
      await expect(service.validateRefreshToken('member-1', 'some-token')).rejects.toThrow(
        UnauthorizedException,
      );
    });
    ...
    it('should throw UnauthorizedException if refresh token is invalid', async () => {
      ...
      await expect(service.validateRefreshToken('member-1', 'invalid-token')).rejects.toThrow(
        UnauthorizedException,
      );
    });
  ...
  describe('validateAccessToken', () => {
    it('should throw UnauthorizedException if no session (hashed token is null)', async () => {
      ...
      await expect(service.validateAccessToken('member-1')).rejects.toThrow(UnauthorizedException);
    });
