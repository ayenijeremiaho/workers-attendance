...
import {CreateChildAgeGroupDto, UpdateChildAgeGroupDto} from '../dto/create-age-group.dto';
import {CreateChildClassGroupDto, UpdateChildClassGroupDto} from '../dto/create-class-group.dto';
import {CreateChildProfileDto, UpdateChildProfileDto} from '../dto/create-child-profile.dto';
...
export class ChildrenChurchService {
    ...
    async createAgeGroup(dto: CreateChildAgeGroupDto): Promise<ChildAgeGroup> {
        ...
        if (dto.minAgeMonths >= dto.maxAgeMonths) {
            throw new BadRequestException('minAgeMonths must be less than maxAgeMonths');
        }
    ...
    async updateAgeGroup(id: string, dto: UpdateChildAgeGroupDto): Promise<ChildAgeGroup> {
        ...
        if (min >= max) throw new BadRequestException('minAgeMonths must be less than maxAgeMonths');
        return this.ageGroupRepo.save(entity);
    }
    ...
    async getAllAgeGroups(): Promise<ChildAgeGroup[]> {
        return this.ageGroupRepo.find({order: {displayOrder: 'ASC', minAgeMonths: 'ASC'}});
    }

    // ─── Class Groups ─────────────────────────────────────────────────────────

    async createClassGroup(dto: CreateChildClassGroupDto): Promise<ChildClassGroup> {
        this.logger.log(`Creating class group "${dto.name}" in age group ${dto.ageGroupId}`);
        const ageGroup = await this.ageGroupRepo.findOne({where: {id: dto.ageGroupId}});
    ...
    async updateClassGroup(id: string, dto: UpdateChildClassGroupDto): Promise<ChildClassGroup> {
        this.logger.log(`Updating class group ${id}`);
        const entity = await this.classGroupRepo.findOne({where: {id}, relations: ['ageGroup']});
        if (!entity) throw new NotFoundException('Class group not found');
        if (dto.name !== undefined) entity.name = dto.name;
        if (dto.capacity !== undefined) entity.capacity = dto.capacity ?? null;
        if (dto.teacherNote !== undefined) entity.teacherNote = dto.teacherNote ?? null;
        return this.classGroupRepo.save(entity);
    }
    ...
    async deleteClassGroup(id: string): Promise<void> {
        ...
        const childCount = await this.childProfileRepo.count({where: {classGroup: {id}}});
        if (childCount > 0) {
            throw new BadRequestException(
    ...
    async getClassGroupsByAgeGroup(ageGroupId: string): Promise<ChildClassGroup[]> {
    ...
    async registerChild(user: MemberAuth, dto: CreateChildProfileDto): Promise<ChildProfile> {
        await this.requireChildrenChurchAuth(user);
        this.logger.log(`Registering child: ${dto.firstname} ${dto.lastname}`);
        const {ageGroup, classGroup} = await this.computeAgeGroup(dto.dateOfBirth);
    ...
    async updateChild(user: MemberAuth, id: string, dto: UpdateChildProfileDto): Promise<ChildProfile> {
        await this.requireChildrenChurchAuth(user);
        ...
        if (dto.specialNotes !== undefined) entity.specialNotes = dto.specialNotes ?? null;
    ...
    async batchRecomputeAgeGroups(): Promise<{ updated: number }> {
        ...
        const [ageGroups, classGroups, children] = await Promise.all([
            this.ageGroupRepo.find({order: {displayOrder: 'ASC', minAgeMonths: 'ASC'}}),
        ...
        const occupancy = new Map<string, number>(classGroups.map((cg) => [cg.id, 0]));
        for (const child of children) {
            if (child.classGroup) {
                occupancy.set(child.classGroup.id, (occupancy.get(child.classGroup.id) ?? 0) + 1);
            }
        }
        ...
        for (const child of children) {
            ...
            const ageGroup =
                ageGroups.find(
                    (ag) => ag.minAgeMonths <= ageInMonths && ag.maxAgeMonths >= ageInMonths,
                ) ?? null;
            ...
            if (ageGroup) {
                ...
                if (candidates.length) {
                    classGroup = candidates.reduce((min, cg) =>
                        (occupancy.get(cg.id) ?? 0) < (occupancy.get(min.id) ?? 0) ? cg : min,
                    );
                    // Adjust occupancy snapshot when the assignment changes
                    if (child.classGroup?.id !== classGroup.id) {
                        occupancy.set(classGroup.id, (occupancy.get(classGroup.id) ?? 0) + 1);
    ...
    async addGuardian(user: MemberAuth, childId: string, dto: CreateGuardianDto): Promise<ChildGuardian> {
        await this.requireChildrenChurchAuth(user);
        this.logger.log(`Adding guardian for child ${childId}`);
        const child = await this.childProfileRepo.findOne({where: {id: childId}});
    ...
    async removeGuardian(user: MemberAuth, guardianId: string): Promise<void> {
        ...
        const entity = await this.guardianRepo.findOne({where: {id: guardianId}});
        if (!entity) throw new NotFoundException('Guardian not found');
        await this.guardianRepo.remove(entity);
    }

    async getChildGuardians(user: MemberAuth, childId: string): Promise<ChildGuardian[]> {
        await this.requireChildrenChurchAuth(user);
        const child = await this.childProfileRepo.findOne({where: {id: childId}});
        if (!child) throw new NotFoundException('Child not found');
    ...
    async checkIn(user: MemberAuth, dto: ChildCheckInDto): Promise<ChildCheckIn> {
        ...
        const activeCheckIn = await this.checkInRepo.findOne({
            where: {child: {id: dto.childId}, status: ChildCheckInStatusEnum.CHECKED_IN},
        });
        if (activeCheckIn) {
            throw new BadRequestException('This child is already checked in for this service.');
        }
        ...
        const entity = this.checkInRepo.create({
            child,
            serviceSlot: dto.serviceSlotId ? ({id: dto.serviceSlotId} as any) : null,
    ...
    async verifyPickupCode(user: MemberAuth, code: string): Promise<ChildCheckIn> {
        await this.requireChildrenChurchAuth(user);
        const checkIn = await this.checkInRepo.findOne({
            where: {pickupCode: code.toUpperCase(), status: ChildCheckInStatusEnum.CHECKED_IN},
            relations: ['child', 'child.classGroup', 'child.ageGroup', 'child.guardians', 'droppedOffBy', 'checkedInBy'],
        });
        if (!checkIn) throw new NotFoundException('This pickup code is invalid or has expired. Please verify and try again.');
        return checkIn;
    }

    async checkOut(user: MemberAuth, dto: ChildCheckOutDto): Promise<ChildCheckIn> {
        await this.requireChildrenChurchAuth(user);
        ...
        if (!checkIn) throw new NotFoundException('This pickup code is invalid or has already been used.');
        ...
        this.sendPickupNotification(saved).catch((err) =>
            this.logger.error(`Failed to send pickup notification for check-in ${saved.id}: ${err.message}`),
        );

        return saved;
    }

    async flagCheckIn(user: MemberAuth, checkInId: string, dto: FlagCheckInDto): Promise<ChildCheckIn> {
        await this.requireChildrenChurchAuth(user);
    ...
    async getActiveCheckIns(user: MemberAuth, classGroupId?: string): Promise<ChildCheckIn[]> {
        await this.requireChildrenChurchAuth(user);
        ...
        return this.checkInRepo.find({
            where,
            relations: ['child', 'child.classGroup', 'child.ageGroup', 'droppedOffBy', 'serviceSlot', 'checkedInBy'],
    ...
    async getCheckInsBySlot(slotId: string): Promise<ChildCheckIn[]> {
        return this.checkInRepo.find({
            ...
            relations: ['child', 'child.classGroup', 'child.ageGroup', 'droppedOffBy', 'pickedUpBy', 'checkedInBy'],
            order: {checkinTime: 'ASC'},
        });
    }
    ...
    private async computeAgeGroup(
        dateOfBirth: string,
    ): Promise<{ ageGroup: ChildAgeGroup | null; classGroup: ChildClassGroup | null }> {
        ...
        const counts = await Promise.all(
            classGroups.map(async (cg) => ({
                classGroup: cg,
                count: await this.childProfileRepo.count({where: {classGroup: {id: cg.id}}}),
            })),
        );
    ...
    private generateCode(): string {
        return Array.from({length: PICKUP_CODE_LENGTH}, () =>
