...
export class ChildrenChurchService {
  ...
  async checkIn(user: MemberAuth, dto: ChildCheckInDto): Promise<ChildCheckIn> {
    ...
    if (activeCheckIn) {
      throw new BadRequestException('Child is already checked in');
    }
  ...
  async verifyPickupCode(user: MemberAuth, code: string): Promise<ChildCheckIn> {
    ...
    if (!checkIn) throw new NotFoundException('Invalid or expired pickup code');
    return checkIn;
  }
  ...
  async checkOut(user: MemberAuth, dto: ChildCheckOutDto): Promise<ChildCheckIn> {
    ...
    if (!checkIn) throw new NotFoundException('Invalid or already used pickup code');
  ...
  private async requireChildrenChurchAuth(user: MemberAuth): Promise<void> {
    ...
    throw new ForbiddenException(
      'Only Children Church department workers can perform this operation',
    );
  }
  ...
  private generateCode(): string {
    return Array.from({ length: PICKUP_CODE_LENGTH }, () =>
      PICKUP_CODE_CHARSET[Math.floor(Math.random() * PICKUP_CODE_CHARSET.length)],
    ).join('');
  }
