...
describe('DepartmentService', () => {
  ...
  describe('assignLead', () => {
    ...
    it('should throw NotFoundException if department does not exist', async () => {
      ...
      await expect(service.assignLead(dto)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw BadRequestException if worker already holds that lead role', async () => {
      ...
      await expect(service.assignLead(dto)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should replace existing lead and assign new one', async () => {
      ...
      mockLeadRepo.create.mockReturnValue({ workerProfile: profile, department, leadType: DepartmentLeadTypeEnum.HOD });
      mockLeadRepo.save.mockResolvedValue({});

      const result = await service.assignLead(dto);
  ...
  describe('removeLead', () => {
    ...
    it('should throw NotFoundException if department does not exist', async () => {
      ...
      await expect(service.removeLead(dto)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw BadRequestException if no lead is assigned', async () => {
      ...
      await expect(service.removeLead(dto)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should remove the lead successfully', async () => {
      ...
      const result = await service.removeLead(dto);
  ...
  describe('getDepartmentSummary', () => {
    const user: MemberAuth = { id: 'member-1', role: MemberRoleEnum.WORKER, requiresPasswordChange: false };
    const lead = {
  ...
  describe('create', () => {
    it('should throw BadRequestException if department name already exists', async () => {
      ...
      await expect(service.create({ name: 'Media', description: '' })).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should create and return a new department', async () => {
      ...
      const result = await service.create({ name: 'Media', description: '' });
  ...
  describe('delete', () => {
    it('should throw NotFoundException if department does not exist', async () => {
      ...
      await expect(service.delete('dept-1')).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw BadRequestException if department has workers', async () => {
      ...
      await expect(service.delete('dept-1')).rejects.toThrow(BadRequestException);
    });
