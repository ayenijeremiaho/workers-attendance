...
import {CreateEventReminderDto, UpdateEventReminderDto} from '../dto/event-reminder.dto';
...
export class EventReminderService {
    ...
    async create(slotId: string, dto: CreateEventReminderDto): Promise<EventReminder> {
        ...
        this.logger.log(`Reminder created for slot ${slotId}: ${dto.intervalPreset} before, audience: ${audience}`);
        return saved;
    }

    async update(reminderId: string, dto: UpdateEventReminderDto): Promise<EventReminder> {
        const reminder = await this.getOrThrow(reminderId);

        const audience = dto.audience ?? reminder.audience;
        if (audience === AnnouncementAudienceEnum.DEPARTMENT && dto.departmentId === undefined && !reminder.department) {
            throw new BadRequestException('departmentId is required for DEPARTMENT audience');
        }
    ...
    private async fireReminder(reminder: EventReminder, now: Date): Promise<void> {
        ...
        const label = minutesBefore >= 60 ? `${hours} hour${hourSuffix}` : `${minutesBefore} minutes`;
        ...
        this.logger.log(`Reminder fired for slot "${slot.name}" (${reminder.intervalPreset}), ${recipients.length} recipients`);
    }
    ...
    private async getRecipientEmails(reminder: EventReminder): Promise<string[]> {
        ...
        } else if (reminder.audience === AnnouncementAudienceEnum.DEPARTMENT && reminder.department) {
            qb.innerJoin('m.workerProfile', 'wp')
                .andWhere('wp.status = :wpStatus', {wpStatus: WorkerStatusEnum.ACTIVE})
                .andWhere('wp.departmentId = :deptId', {deptId: reminder.department.id});
        }
