...
import {AccountReportQueryDto, BudgetActualsQueryDto, MemberGivingQueryDto, PeriodReportQueryDto, PledgeSummaryQueryDto} from '../dto/report.dto';
...
export class FinanceReportController {
    ...
    cashFlow(@Query() query: AccountReportQueryDto) {
        return this.reportService.cashFlow(query.accountId, query.fromDate, query.toDate);
    }
    ...
    accountLedger(@Query() query: AccountReportQueryDto) {
        return this.reportService.accountLedger(query.accountId, query.fromDate, query.toDate);
    }
