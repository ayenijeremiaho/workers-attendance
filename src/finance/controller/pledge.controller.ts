...
import {CreatePledgeCampaignDto, CreatePledgeDto, PledgeQueryDto, UpdatePledgeStatusDto} from '../dto/pledge.dto';
...
export class PledgeController {
    ...
    @Post('campaigns')
    createCampaign(@Body() dto: CreatePledgeCampaignDto, @CurrentAdmin() admin: Admin) {
    ...
    @RequiresPermission(AdminPermission.FINANCE_WRITE)
    @Patch(':id/status')
    updateStatus(@Param('id', ParseUUIDPipe) id: string, @Body() dto: UpdatePledgeStatusDto, @CurrentAdmin() admin: Admin) {
