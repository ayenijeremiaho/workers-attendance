...
import {JournalEntrySource, JournalEntryStatus, JournalEntryType} from '../enum/finance.enum';
...
export class JournalEntry extends BaseEntity {
    ...
    @Column({type: 'varchar', default: JournalEntryType.STANDARD, name: 'entry_type'})
    ...
    @Column({type: 'numeric', precision: 15, scale: 6, nullable: true, name: 'exchange_rate'})
    exchangeRate: number | null;

    @Column({type: 'numeric', precision: 15, scale: 2, nullable: true, name: 'original_amount'})
    originalAmount: number | null;

    @OneToMany(() => JournalEntryLine, line => line.journalEntry, {cascade: true})
    lines: JournalEntryLine[];

    @OneToMany(() => JournalEntryLink, link => link.journalEntry, {cascade: true})
