...
import {BulkUploadJobStatus, ReconciliationRowStatus} from '../enum/finance.enum';
...
export class ReconciliationProcessor implements OnApplicationBootstrap {
    ...
    async handleParse(job: Job<ReconciliationJobData>): Promise<void> {
        ...
        const uploadJob = await this.jobRepo.findOne({where: {id: jobId}, relations: ['profile']});
        if (!uploadJob) return;
        ...
        try {
            ...
            if (!profile) {
                await this.jobRepo.update(jobId, {status: BulkUploadJobStatus.FAILED, errorMessage: 'No import profile attached to this job.'});
                return;
            }
            ...
            for (const row of rows) {
                const rowFingerprint = this.fingerprint(`${row.date}|${row.narration}|${row.amount}|${row.creditDebit}`);
                const transactionFingerprint = this.fingerprint(`${row.date}|${row.amount}|${row.creditDebit}`);

                if (existingRowFps.has(rowFingerprint) || existingTxFps.has(transactionFingerprint)) continue;
            ...
            this.logger.log(`Reconciliation job ${jobId} parsed ${rows.length} rows, ${rowEntities.length} new rows queued for review`);
        } catch (err) {
            await this.jobRepo.update(jobId, {
    ...
    async onApplicationBootstrap(): Promise<void> {
        const stuck = await this.jobRepo.count({where: {status: BulkUploadJobStatus.PARSING}});
        if (stuck > 0) {
            await this.jobRepo.update(
                {status: BulkUploadJobStatus.PARSING},
                {status: BulkUploadJobStatus.FAILED, errorMessage: 'Interrupted — service restarted'},
            );
            this.logger.warn(`Reset ${stuck} stuck PARSING reconciliation job(s) to FAILED on startup`);
        }
    }
    ...
    async onStalled(job: Job<ReconciliationJobData>): Promise<void> {
        ...
        await this.jobRepo.update(job.data.jobId, {status: BulkUploadJobStatus.FAILED, errorMessage: 'Job stalled; will be retried automatically'});
    }

    @OnQueueFailed()
    onFailed(job: Job<ReconciliationJobData>, error: Error): void {
        this.logger.error(`Reconciliation job ${job.data.jobId} failed: ${error.message}`);
    }
