...
export class BudgetAlertScheduler {
    ...
    async dispatchBudgetAlerts(): Promise<void> {
        const acquired = await this.cacheService.acquireLock(BudgetAlertScheduler.LOCK_KEY, 300);
        if (!acquired) return;
    ...
    private async runAlerts(): Promise<void> {
        ...
        for (const row of actualsRows) {
            ...
            if (!budget) continue;
            const isNormal = row.entryType === (budget.account.normalBalance as string);
            const current = actualsMap.get(row.budgetId) ?? 0;
            actualsMap.set(row.budgetId, current + (isNormal ? Number(row.total) : -Number(row.total)));
        }

        for (const budget of budgets) {
            try {
                await this.processBudget(budget, actualsMap.get(budget.id) ?? 0, recipients);
            } catch (err) {
                this.logger.error(`Failed to process budget alert for budget ${budget.id}`, err);
            }
        }
    }

    private async processBudget(budget: Budget, actuals: number, recipients: string[]): Promise<void> {
    ...
    private sendAlert(recipients: string[], budget: Budget, actuals: number, thresholdPct: number): void {
    ...
    private async fetchRecipients(): Promise<string[]> {
        ...
        return admins
            .filter(a => a.adminRole?.permissions?.includes(AdminPermission.FINANCE_READ))
