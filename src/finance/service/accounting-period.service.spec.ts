...
describe('AccountingPeriodService', () => {
  ...
  describe('create', () => {
    ...
    it('throws ConflictException for duplicate period', async () => {
      ...
      await expect(
        service.create({ year: 2026, month: 1 }, mockAdmin),
  ...
  describe('close', () => {
    ...
    it('throws ConflictException when already closed', async () => {
      ...
      mockPeriodRepo.findOne.mockResolvedValue(period);
      await expect(service.close('p-1', mockAdmin)).rejects.toThrow(
        ConflictException,
      );
    });
  });
  ...
  describe('reopen', () => {
    it('throws ConflictException when already open', async () => {
      ...
      mockPeriodRepo.findOne.mockResolvedValue(period);
      await expect(service.reopen('p-1', mockAdmin)).rejects.toThrow(
        ConflictException,
      );
    });
