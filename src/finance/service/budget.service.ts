...
export class BudgetService {
    ...
    async create(dto: CreateBudgetDto, admin: Admin): Promise<Budget> {
        ...
        this.auditLogService.log('BUDGET_CREATED', {actorId: admin.id, targetId: saved.id, metadata: {name: saved.name}});
        return saved;
    }
    ...
    async findAll(query: BudgetQueryDto): Promise<PaginationResponseDto<Budget>> {
        ...
        if (isActive !== undefined) qb.andWhere('b.isActive = :isActive', {isActive});
    ...
    async findOne(id: string): Promise<Budget> {
        const budget = await this.budgetRepo.findOne({where: {id}, relations: ['fund', 'account', 'createdBy']});
    ...
    async deactivate(id: string, admin: Admin): Promise<Budget> {
        ...
        const saved = await this.budgetRepo.save(budget);
        this.auditLogService.log('BUDGET_DEACTIVATED', {actorId: admin.id, targetId: saved.id});
        return saved;
    }
}
