...
describe('ExternalPayeeService', () => {
    ...
    describe('create', () => {
        it('creates a new external payee', async () => {
            ...
            const payee = {id: 'ep-1', name: 'Global Mission', type: ExternalPayeeType.MISSION};
            mockRepo.create.mockReturnValue(payee);
            mockRepo.save.mockResolvedValue(payee);

            const result = await service.create({name: 'Global Mission', type: ExternalPayeeType.MISSION}, mockAdmin);
            expect(result.id).toBe('ep-1');
        });

        it('throws ConflictException on duplicate name', async () => {
            mockRepo.findOne.mockResolvedValue({id: 'ep-1'});
            await expect(service.create({name: 'Global Mission', type: ExternalPayeeType.MISSION}, mockAdmin)).rejects.toThrow(ConflictException);
        });
    });
