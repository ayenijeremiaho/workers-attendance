...
import {AccountType, JournalEntryStatus, JournalLineType, PettyCashReplenishmentStatus, PledgeStatus} from '../enum/finance.enum';
...
export class FinanceReportService {
    ...
    async incomeExpense(periodId?: string, fundId?: string): Promise<object> {
        ...
        if (periodId) qb.andWhere('je.accounting_period_id = :periodId', {periodId});
        if (fundId) qb.andWhere('a.fund_id = :fundId', {fundId});

        const lines = await qb
            .select(['a.type as account_type', 'a.name as account_name', 'l.entryType as entry_type', 'SUM(l.amount) as total'])
    ...
    async cashFlow(accountId: string, fromDate?: string, toDate?: string): Promise<object> {
        ...
        return {account: {id: account.id, name: account.name, currentBalance: account.currentBalance}, lines, generatedAt: new Date()};
    }

    async trialBalance(periodId?: string): Promise<object> {
        if (!periodId) {
            const accounts = await this.accountRepo.find({relations: ['fund'], order: {type: 'ASC', name: 'ASC'}});
        ...
        const rows = await this.lineRepo
            ...
            .groupBy('a.id, a.name, a.type, a.normal_balance, f.id, f.name, l.entryType')
            .getRawMany<{accountId: string; accountName: string; accountType: string; normalBalance: string; fundId: string; fundName: string; entryType: string; total: string}>();

        const accountMap = new Map<string, {id: string; name: string; type: string; normalBalance: string; fund: {id: string; name: string}; balance: number}>();
        for (const row of rows) {
            const existing = accountMap.get(row.accountId) ?? {
                id: row.accountId, name: row.accountName, type: row.accountType,
                normalBalance: row.normalBalance, fund: {id: row.fundId, name: row.fundName}, balance: 0,
            };
        ...
        return {accounts: Array.from(accountMap.values()), periodId, generatedAt: new Date()};
    }
    ...
    async fundBalance(): Promise<object> {
        const rows = await this.accountRepo
            ...
            .orderBy('f.name', 'ASC')
            .getRawMany<{fundId: string; fundName: string; fundType: string; totalBalance: string; accountCount: string}>();
    ...
    async accountLedger(accountId: string, fromDate?: string, toDate?: string): Promise<object> {
        const account = await this.accountRepo.findOne({where: {id: accountId}, relations: ['fund']});
        if (!account) throw new NotFoundException('Account not found.');
    ...
    async budgetActuals(budgetId: string): Promise<object> {
        const budget = await this.budgetRepo.findOne({where: {id: budgetId}, relations: ['fund', 'account', 'createdBy']});
        ...
        return {budget, actuals, variance: Number(budget.amount) - actuals, generatedAt: new Date()};
    }

    async pledgeSummary(campaignId: string): Promise<object> {
        const campaign = await this.campaignRepo.findOne({where: {id: campaignId}, relations: ['fund', 'createdBy']});
        if (!campaign) throw new NotFoundException('Campaign not found.');

        const pledges = await this.pledgeRepo.find({where: {campaign: {id: campaignId}}, relations: ['member']});
        const totalPledged = pledges.reduce((s, p) => s + Number(p.totalAmount), 0);

        return {campaign, pledges, totalPledged, pledgeCount: pledges.length, generatedAt: new Date()};
    }
    ...
    async dashboard(): Promise<object> {
        ...
        const [mtdRows, pendingJournalEntries, pendingPettyCash, activeBudgets, outstandingPledges] = await Promise.all([
            this.lineRepo
                .createQueryBuilder('l')
                .innerJoin('l.journalEntry', 'je')
                .innerJoin('l.account', 'a')
                ...
                .select(['a.type AS account_type', 'l.entryType AS entry_type', 'SUM(l.amount) AS total'])
                .groupBy('a.type, l.entryType')
                .getRawMany(),

            this.entryRepo.count({where: {status: JournalEntryStatus.PENDING_APPROVAL}}),

            this.pettyCashRepo.count({where: {status: PettyCashReplenishmentStatus.PENDING}}),

            this.budgetRepo.find({where: {isActive: true}, relations: ['account']}),
        ...
        const mtdIncome = mtdRows
            .filter(r => r.account_type === AccountType.INCOME && r.entry_type === JournalLineType.CREDIT)
            .reduce((s, r) => s + Number(r.total), 0);

        const mtdExpenses = mtdRows
            .filter(r => r.account_type === AccountType.EXPENSE && r.entry_type === JournalLineType.DEBIT)
            .reduce((s, r) => s + Number(r.total), 0);

        const actualsRows = await this.lineRepo
            .createQueryBuilder('l')
            .innerJoin('l.journalEntry', 'je')
            .innerJoin('finance_budgets', 'b', 'b.account_id = l.account_id AND b.is_active = true AND je.date >= b.start_date AND je.date <= b.end_date')
            .where('je.status = :status', {status: JournalEntryStatus.POSTED})
            .andWhere('b.id IN (:...budgetIds)', {budgetIds: activeBudgets.length > 0 ? activeBudgets.map(b => b.id) : ['__none__']})
        ...
        for (const row of actualsRows) {
            ...
            if (row.entryType === JournalLineType.DEBIT) existing.debit += Number(row.total);
        ...
        const budgetUtilizations = activeBudgets.map(budget => {
            ...
            const isDebitNormal = (budget.account.normalBalance as string) === JournalLineType.DEBIT;
            const actuals = isDebitNormal ? data.debit - data.credit : data.credit - data.debit;
            const utilizationPct = Number(budget.amount) > 0 ? Math.round((actuals / Number(budget.amount)) * 100) : 0;
            return {budgetId: budget.id, name: budget.name, amount: Number(budget.amount), actuals, utilizationPct};
        });
        ...
        return {
            ...
            pendingPettyCash,
            budgetsNearLimit: budgetUtilizations.filter(b => b.utilizationPct >= 80).sort((a, b) => b.utilizationPct - a.utilizationPct),
    ...
    async memberGiving(memberId: string, periodId?: string): Promise<object> {
        const qb = this.lineRepo
            .createQueryBuilder('l')
            .innerJoin('l.journalEntry', 'je')
            .innerJoin('finance_journal_entry_links', 'lk', 'lk.journal_entry_id = je.id AND lk.member_id = :memberId', {memberId})
            .where('je.status = :status', {status: JournalEntryStatus.POSTED})
            .orderBy('je.date', 'ASC');
        ...
        return {memberId, periodId: periodId ?? null, lines, total, generatedAt: new Date()};
    }
}
