...
describe('FundService', () => {
    ...
    describe('create', () => {
        it('creates a new fund', async () => {
            ...
            const result = await service.create({name: 'General', type: FundType.UNRESTRICTED}, mockAdmin);
            expect(result).toEqual(fund);
            expect(mockAuditLogService.log).toHaveBeenCalledWith('FUND_CREATED', expect.any(Object));
        });

        it('throws ConflictException when name already exists', async () => {
            mockFundRepo.findOne.mockResolvedValue({id: 'f-1', name: 'General'});
            await expect(service.create({name: 'General', type: FundType.UNRESTRICTED}, mockAdmin)).rejects.toThrow(ConflictException);
        });
    });
    ...
    describe('update', () => {
        it('updates fund fields', async () => {
            const fund = {id: 'f-1', name: 'General', description: null, isActive: true};
            mockFundRepo.findOne.mockResolvedValue(fund);
            mockFundRepo.save.mockResolvedValue({...fund, name: 'Updated'});

            const result = await service.update('f-1', {name: 'Updated'}, mockAdmin);
            expect(result.name).toBe('Updated');
            expect(mockAuditLogService.log).toHaveBeenCalledWith('FUND_UPDATED', expect.any(Object));
        });
    });
});
