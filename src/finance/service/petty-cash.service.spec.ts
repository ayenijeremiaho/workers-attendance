...
describe('PettyCashService', () => {
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                PettyCashService,
                {provide: getRepositoryToken(PettyCashReplenishment), useValue: mockRepo},
    ...
    describe('create', () => {
        it('creates a pending replenishment request', async () => {
            const r = {id: 'r-1', amount: 5000, status: PettyCashReplenishmentStatus.PENDING};
            mockRepo.create.mockReturnValue(r);
            mockRepo.save.mockResolvedValue(r);

            const result = await service.create({fromAccountId: 'a-1', toCashAccountId: 'a-2', amount: 5000}, mockAdmin);
    ...
    describe('approve', () => {
        it('approves a pending replenishment and creates journal entry', async () => {
            ...
            mockRepo.save.mockResolvedValue({...r, status: PettyCashReplenishmentStatus.APPROVED, approvedBy: mockAdmin});
        ...
        it('skips journal entry creation if already exists (idempotent)', async () => {
            ...
            mockRepo.findOne.mockResolvedValue(r);
            mockRepo.save.mockResolvedValue({...r, status: PettyCashReplenishmentStatus.APPROVED});
        ...
        it('prevents self-approval', async () => {
            const r = {id: 'r-1', status: PettyCashReplenishmentStatus.PENDING, requestedBy: {id: 'admin-1'}};
            mockRepo.findOne.mockResolvedValue(r);
            await expect(service.approve('r-1', {}, mockAdmin)).rejects.toThrow(BadRequestException);
        });

        it('throws BadRequestException when not pending', async () => {
            const r = {id: 'r-1', status: PettyCashReplenishmentStatus.APPROVED, requestedBy: {id: 'admin-2'}};
            mockRepo.findOne.mockResolvedValue(r);
            await expect(service.approve('r-1', {}, mockAdmin)).rejects.toThrow(BadRequestException);
        });
        ...
        it('throws NotFoundException when not found', async () => {
            ...
            await expect(service.approve('missing', {}, mockAdmin)).rejects.toThrow(NotFoundException);
        });
    });
});
