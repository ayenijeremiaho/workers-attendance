...
import {AccountingPeriodStatus, JournalEntrySource, JournalEntryStatus, JournalEntryType, JournalLineType, PettyCashReplenishmentStatus} from '../enum/finance.enum';
...
export class PettyCashService {
    ...
    async create(dto: CreatePettyCashReplenishmentDto, admin: Admin): Promise<PettyCashReplenishment> {
        ...
        this.auditLogService.log('PETTY_CASH_REQUESTED', {actorId: admin.id, targetId: saved.id, metadata: {amount: saved.amount}});
        return saved;
    }

    async findAll(page = 1, limit = 20): Promise<PaginationResponseDto<PettyCashReplenishment>> {
    ...
    async findOne(id: string): Promise<PettyCashReplenishment> {
        const r = await this.replenishmentRepo.findOne({where: {id}, relations: ['fromAccount', 'toCashAccount', 'requestedBy', 'approvedBy']});
        if (!r) throw new NotFoundException('Petty cash replenishment not found.');
        return r;
    }

    async approve(id: string, dto: ApprovePettyCashDto, admin: Admin): Promise<PettyCashReplenishment> {
        const r = await this.findOne(id);
        if (r.status !== PettyCashReplenishmentStatus.PENDING) throw new BadRequestException('Only pending replenishments can be approved.');
        if (r.requestedBy?.id === admin.id) throw new BadRequestException('You cannot approve your own replenishment request.');
        ...
        const [existing, period] = await Promise.all([
            ...
            this.periodRepo.findOne({
                where: {year: new Date().getFullYear(), month: new Date().getMonth() + 1, status: AccountingPeriodStatus.OPEN},
            }),
        ]);
        if (!existing && !period) {
            throw new BadRequestException('No open accounting period for the current month — cannot create journal entry.');
        }
        ...
        this.auditLogService.log('PETTY_CASH_APPROVED', {actorId: admin.id, targetId: saved.id});
        return saved;
    }
}
