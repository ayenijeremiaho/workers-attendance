...
import {CreateRecurringEntryDto, UpdateRecurringEntryDto} from '../dto/recurring-entry.dto';
...
export class RecurringEntryService {
    ...
    async create(dto: CreateRecurringEntryDto, admin: Admin): Promise<RecurringEntry> {
        ...
        this.auditLogService.log('RECURRING_ENTRY_CREATED', {actorId: admin.id, targetId: saved.id, metadata: {description: saved.description}});
        return saved;
    }
    ...
    async findOne(id: string): Promise<RecurringEntry> {
        const entry = await this.recurringRepo.findOne({where: {id}, relations: ['debitAccount', 'creditAccount', 'fund', 'createdBy']});
        if (!entry) throw new NotFoundException('Recurring entry not found.');
        return entry;
    }

    async update(id: string, dto: UpdateRecurringEntryDto, admin: Admin): Promise<RecurringEntry> {
        ...
        const saved = await this.recurringRepo.save(entry);
        this.auditLogService.log('RECURRING_ENTRY_UPDATED', {actorId: admin.id, targetId: saved.id, metadata: dto as unknown as Record<string, unknown>});
        return saved;
    }
}
