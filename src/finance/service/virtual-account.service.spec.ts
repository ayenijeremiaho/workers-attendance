...
describe('VirtualAccountService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(MemberVirtualAccount),
          useValue: mockVaRepo,
        },
        {
  ...
  it('requestAccount throws NotImplementedException', async () => {
    await expect(
      service.requestAccount('member-1', {
  ...
  it('getMyAccounts throws NotImplementedException', async () => {
    await expect(service.getMyAccounts('member-1')).rejects.toThrow(
  ...
  it('deactivate throws NotImplementedException', async () => {
    await expect(
      service.deactivate('va-1', { id: 'admin-1' } as any),
    ).rejects.toThrow(NotImplementedException);
  });

  it('handleWebhookCredit throws NotImplementedException', async () => {
    await expect(
      service.handleWebhookCredit(Buffer.from('{}'), 'sig'),
    ).rejects.toThrow(NotImplementedException);
  });
});
