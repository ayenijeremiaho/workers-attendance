...
export class CsvParser {
    ...
    switch (profile.amountConvention) {
      case AmountConvention.SIGNED: {
        ...
        return {
          ...
          creditDebit: value < 0 ? 'DEBIT' : 'CREDIT',
        };
      }
      case AmountConvention.SEPARATE_COLUMNS: {
        ...
        if (!hasDebit && !hasCredit) {
          return {
            ...
            found: `debit=${debitRaw}, credit=${creditRaw}`,
          };
        }
        return {
          date,
          ...
          creditDebit: hasDebit ? 'DEBIT' : 'CREDIT',
        };
      }
      case AmountConvention.AMOUNT_WITH_TYPE: {
        ...
        const debitIndicator = (
          profile.debitIndicator ?? 'DEBIT'
        ).toUpperCase();
        const creditIndicator = (
          profile.creditIndicator ?? 'CREDIT'
        ).toUpperCase();
        if (typeUpper !== debitIndicator && typeUpper !== creditIndicator) {
          return {
            row: -1,
            column: 'type',
            expected: `${debitIndicator}/${creditIndicator}`,
            found: typeRaw,
          };
        }
        return {
          ...
          creditDebit: typeUpper === debitIndicator ? 'DEBIT' : 'CREDIT',
        };
      }
    }
  }
  ...
  static buildTemplateHeaders(profile: BankImportProfile): string[] {
    ...
    headers[profile.narrationColumnIndex] =
      profile.narrationColumnName ?? 'Narration';
    switch (profile.amountConvention) {
      case AmountConvention.SIGNED:
        headers[profile.amountColumnIndex!] =
          profile.amountColumnName ?? 'Amount';
        break;
      case AmountConvention.SEPARATE_COLUMNS:
        headers[profile.debitColumnIndex!] = profile.debitColumnName ?? 'Debit';
        headers[profile.creditColumnIndex!] =
          profile.creditColumnName ?? 'Credit';
        break;
      case AmountConvention.AMOUNT_WITH_TYPE:
        headers[profile.amountColumnIndex!] =
          profile.amountColumnName ?? 'Amount';
