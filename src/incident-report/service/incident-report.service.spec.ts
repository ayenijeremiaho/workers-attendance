...
describe('IncidentReportService', () => {
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                {provide: getRepositoryToken(IncidentReport), useValue: mockReportRepo},
    ...
    describe('create', () => {
        it('creates and saves an incident report', async () => {
            const dto = {title: 'Broken window', description: 'Window in hall B is broken'};
            const created = {id: 'uuid-1', ...dto, isAnonymous: false, images: null, location: null, reporter: {id: 'member-1'}};
        ...
        it('throws 429 when daily limit exceeded', async () => {
            ...
            await expect(service.create(dto, 'member-1')).rejects.toThrow(HttpException);
        });
    ...
    describe('findMyReports', () => {
        it('returns only reports belonging to the requesting member', async () => {
            const reports = [{id: 'uuid-1', reporter: {id: 'member-1'}}, {id: 'uuid-2', reporter: {id: 'member-1'}}];
    ...
    describe('findMyReport', () => {
        it('returns the report when it belongs to the member', async () => {
            ...
            expect(mockReportRepo.findOne).toHaveBeenCalledWith(
                expect.objectContaining({where: {id: 'uuid-1', reporter: {id: 'member-1'}}}),
            );
        });
        ...
        it('throws NotFoundException when report does not belong to the member', async () => {
            ...
            await expect(service.findMyReport('uuid-1', 'other-member')).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('updateStatus', () => {
        it('updates status and saves', async () => {
            const report = {id: 'uuid-1', status: IncidentStatus.OPEN, adminNotes: null, resolvedAt: null, reporter: null, isAnonymous: false} as any;
            mockReportRepo.findOne.mockResolvedValue(report);
            mockReportRepo.save.mockResolvedValue({...report, status: IncidentStatus.IN_PROGRESS});

            await service.updateStatus('uuid-1', {status: IncidentStatus.IN_PROGRESS}, mockAdmin);
        ...
        it('sets resolvedAt when status is RESOLVED', async () => {
            const report = {id: 'uuid-1', status: IncidentStatus.IN_PROGRESS, adminNotes: null, resolvedAt: null, reporter: null, isAnonymous: false} as any;
            mockReportRepo.findOne.mockResolvedValue(report);
            mockReportRepo.save.mockResolvedValue(report);

            await service.updateStatus('uuid-1', {status: IncidentStatus.RESOLVED}, mockAdmin);
        ...
        it('updates adminNotes when provided', async () => {
            ...
            await service.updateStatus('uuid-1', {status: IncidentStatus.IN_PROGRESS, adminNotes: 'Being investigated'}, mockAdmin);
    ...
    describe('maskReporter', () => {
        it('returns reporter when isAnonymous is false', () => {
            const report = {id: 'uuid-1', isAnonymous: false, reporter: {id: 'member-1', firstname: 'John'}} as any;
        ...
        it('strips reporter when isAnonymous is true', () => {
            const report = {id: 'uuid-1', isAnonymous: true, reporter: {id: 'member-1', firstname: 'John'}} as any;
