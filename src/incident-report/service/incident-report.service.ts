...
import {CreateIncidentReportDto, UpdateIncidentStatusDto} from '../dto/incident-report.dto';
...
export class IncidentReportService {
    ...
    async create(dto: CreateIncidentReportDto, memberId: string): Promise<IncidentReport> {
        const limit = this.configService.get<number>('INCIDENT_DAILY_REPORT_LIMIT', 2);
        const count = await this.cacheService.incr(this.rateLimitKey(memberId), 86400);
        if (count > limit) {
    ...
    async findMyReports(memberId: string, page: number, limit: number): Promise<PaginationResponseDto<IncidentReport>> {
    ...
    async findMyReport(id: string, memberId: string): Promise<IncidentReport> {
        const report = await this.reportRepo.findOne({where: {id, reporter: {id: memberId}}});
        if (!report) throw new NotFoundException('Incident report not found');
        return report;
    }

    async findAll(page: number, limit: number): Promise<PaginationResponseDto<IncidentReport>> {
        const [reports, total] = await this.reportRepo.findAndCount({
            relations: ['reporter'],
            order: {createdAt: 'DESC'},
            skip: (page - 1) * limit,
    ...
    async updateStatus(id: string, dto: UpdateIncidentStatusDto, admin: Admin): Promise<IncidentReport> {
    ...
    private async notifyAdmins(report: IncidentReport): Promise<void> {
        ...
        const recipients = admins
            .filter(a => a.adminRole?.permissions?.includes(AdminPermission.INCIDENT_REPORT_WRITE))
