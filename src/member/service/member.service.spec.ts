...
describe('MemberService', () => {
    ...
    describe('promoteToWorker', () => {
        it('should throw BadRequestException if member is already a worker', async () => {
            ...
            await expect(service.promoteToWorker('member-1', {departmentId: 'dept-1'} as any, 'actor-1')).rejects.toThrow(
                BadRequestException,
            );
        });
        ...
        it('should throw NotFoundException if department not found', async () => {
            ...
            await expect(service.promoteToWorker('member-1', {departmentId: 'nonexistent-dept'} as any, 'actor-1')).rejects.toThrow(
                NotFoundException,
            );
        });
    ...
    describe('revokeWorker', () => {
        it('should throw BadRequestException if member is not a worker', async () => {
            ...
            await expect(service.revokeWorker('member-1', 'actor-1')).rejects.toThrow(BadRequestException);
        });
        ...
        it('should throw NotFoundException when member does not exist', async () => {
            ...
            await expect(service.revokeWorker('nonexistent', 'actor-1')).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('changeStatus', () => {
        it('should throw BadRequestException if status is unchanged', async () => {
            ...
            await expect(service.changeStatus('member-1', MemberStatusEnum.ACTIVE, 'actor-1')).rejects.toThrow(BadRequestException);
        });
        ...
        it('should update member status when different', async () => {
            ...
            await service.changeStatus('member-1', MemberStatusEnum.INACTIVE, 'actor-1');
        ...
        it('should throw NotFoundException when member not found for status change', async () => {
            ...
            await expect(service.changeStatus('nonexistent', MemberStatusEnum.INACTIVE, 'actor-1')).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('getById', () => {
        ...
        it('should return member when found', async () => {
            const member = {
                ...
                email: 'test@test.com',
                role: MemberRoleEnum.MEMBER,
                requiresPasswordChange: false, surface: SessionSurface.MEMBER};
            mockMemberRepo.findOne.mockResolvedValue(member);
