...
export class MemberService {
    ...
    async signup(dto: SignupDto): Promise<Member> {
        ...
        this.auditLogService.log('MEMBER_SIGNED_UP', {targetId: saved.id, targetEmail: saved.email});
    ...
    async promoteToWorker(memberId: string, dto: PromoteToWorkerDto, actorId: string): Promise<Member> {
        const member = await this.getById(memberId, ['workerProfile']);

        if (member.workerProfile) {
            throw new BadRequestException('This member is already registered as a worker.');
        }
        ...
        this.utilityService.sendEmailWithTemplate(
            ...
            'welcome-worker',
            {
                ...
                explainer_video_android_url: this.configService.get<string>('EXPLAINER_VIDEO_ANDROID_URL'),
                explainer_video_ios_url: this.configService.get<string>('EXPLAINER_VIDEO_IOS_URL'),
    ...
    async revokeWorker(memberId: string, actorId: string): Promise<void> {
        const member = await this.getById(memberId, ['workerProfile']);
        if (!member.workerProfile) throw new BadRequestException('This member is not registered as a worker.');
        ...
        this.auditLogService.log('WORKER_REVOKED', {actorId, targetId: member.id, targetEmail: member.email});
    ...
    async updateMember(id: string, dto: UpdateMemberDto, actorId: string): Promise<Member> {
    ...
    async updateWorkerProfile(memberId: string, dto: UpdateWorkerProfileDto, actorId: string): Promise<WorkerProfile> {
        const member = await this.getById(memberId, [
            'workerProfile',
            'workerProfile.department',
            'workerProfile.secondaryDepartment',
        ]);
        if (!member.workerProfile) throw new BadRequestException('This member does not have a worker profile.');
    ...
    async changeStatus(memberId: string, status: MemberStatusEnum, actorId: string): Promise<void> {
        const member = await this.getById(memberId);
        if (member.status === status) {
            throw new BadRequestException(`This member's account is already ${status.toLowerCase()}.`);
        }
        member.status = status;
        await this.memberRepository.save(member);
        this.logger.log(`Member ${memberId} status changed to ${status} by actor ${actorId}`);
        this.auditLogService.log(status === MemberStatusEnum.INACTIVE ? 'MEMBER_DEACTIVATED' : 'MEMBER_ACTIVATED', {
            actorId,
            targetId: member.id,
            targetEmail: member.email
        });
    ...
    async resetPassword(memberId: string, actorId: string): Promise<string> {
        ...
        await this.memberRepository.save(member);
        this.logger.log(`Password reset by admin ${actorId} for member ${memberId}`);
        this.auditLogService.log('ADMIN_PASSWORD_RESET', {actorId, targetId: member.id, targetEmail: member.email});
    ...
    async purgeDevice(memberId: string, actorId: string): Promise<void> {
        ...
        this.logger.log(`Device lock purged for member ${memberId} by actor ${actorId}`);
