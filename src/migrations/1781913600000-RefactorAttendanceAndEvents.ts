...
export class RefactorAttendanceAndEvents1781913600000 implements MigrationInterface {
    async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`UPDATE events SET end_date = event_date WHERE end_date IS NULL`);
        await queryRunner.query(`ALTER TABLE events ALTER COLUMN end_date SET NOT NULL`);

        // ── service_slots: drop marked_absent ────────────────────────────────────
        await queryRunner.query(`ALTER TABLE service_slots DROP COLUMN IF EXISTS marked_absent`);
        ...
        `);
        await queryRunner.query(`ALTER TABLE attendances ALTER COLUMN event_id SET NOT NULL`);
        await queryRunner.query(`
            ALTER TABLE attendances
        ...
        await queryRunner.query(`ALTER TABLE attendances ALTER COLUMN service_slot_id DROP NOT NULL`);
    ...
    async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE members RENAME COLUMN date_joined_church TO year_joined_church`);
        ...
        `);
        await queryRunner.query(`ALTER TABLE attendances DROP CONSTRAINT IF EXISTS fk_attendances_service_slot`);
        await queryRunner.query(`ALTER TABLE attendances DROP CONSTRAINT IF EXISTS fk_attendances_event`);
        await queryRunner.query(`ALTER TABLE attendances DROP COLUMN IF EXISTS event_id`);
        await queryRunner.query(`ALTER TABLE attendances ALTER COLUMN service_slot_id SET NOT NULL`);

        await queryRunner.query(`ALTER TABLE service_slots ADD COLUMN IF NOT EXISTS marked_absent BOOLEAN NOT NULL DEFAULT false`);

        await queryRunner.query(`ALTER TABLE events DROP COLUMN IF EXISTS attendance_marked`);
        await queryRunner.query(`ALTER TABLE events DROP COLUMN IF EXISTS end_date`);
    }
}
