...
export class CreateFinanceAccounts1783814400000 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        `);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_fin_accounts_fund ON finance_accounts(fund_id)`);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_fin_accounts_type_active ON finance_accounts(type, is_active)`);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_fin_accounts_subtype ON finance_accounts(subtype)`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`DROP INDEX IF EXISTS idx_fin_accounts_subtype`);
        await queryRunner.query(`DROP INDEX IF EXISTS idx_fin_accounts_type_active`);
