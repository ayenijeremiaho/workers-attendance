...
export class CreateFinanceBudgets1784160000000 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        `);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_budgets_fund ON finance_budgets(fund_id)`);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_budgets_account ON finance_budgets(account_id)`);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_budgets_period_active ON finance_budgets(period, is_active)`);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_budgets_date_range ON finance_budgets(start_date, end_date)`);
    }
