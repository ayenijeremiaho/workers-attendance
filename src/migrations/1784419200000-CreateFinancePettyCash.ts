...
export class CreateFinancePettyCash1784419200000 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        `);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_petty_cash_status ON finance_petty_cash_replenishments(status)`);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_petty_cash_requested_by ON finance_petty_cash_replenishments(requested_by_id)`);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_petty_cash_from_account ON finance_petty_cash_replenishments(from_account_id)`);
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_petty_cash_to_account ON finance_petty_cash_replenishments(to_cash_account_id)`);
    }
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP TABLE IF EXISTS finance_petty_cash_replenishments`);
    }
}
