...
const mockUser = { id: 'worker-1', role: MemberRoleEnum.WORKER, requiresPasswordChange: false };
...
describe('SundaySchoolController', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [{ provide: SundaySchoolService, useValue: mockSundaySchoolService }],
    }).compile();
  ...
  it('createClass — passes req.user and dto to service', async () => {
    ...
    mockSundaySchoolService.createClass.mockResolvedValue({ id: 'class-1', ...dto });

    await controller.createClass(mockReq, dto as any);

    expect(mockSundaySchoolService.createClass).toHaveBeenCalledWith(mockUser, dto);
  });

  it('updateClass — passes req.user, id, and dto', async () => {
    const dto = { name: 'Advanced' };
    mockSundaySchoolService.updateClass.mockResolvedValue({ id: 'class-1', ...dto });

    await controller.updateClass(mockReq, 'class-1', dto as any);

    expect(mockSundaySchoolService.updateClass).toHaveBeenCalledWith(mockUser, 'class-1', dto);
  });
  ...
  it('assignMember — passes req.user, classId, and dto', async () => {
    ...
    expect(mockSundaySchoolService.assignMember).toHaveBeenCalledWith(mockUser, 'class-1', dto);
  });
  ...
  it('getClassMembers — coerces page and limit', async () => {
    ...
    expect(mockSundaySchoolService.getClassMembers).toHaveBeenCalledWith('class-1', 1, 20);
  });

  it('createSession — passes req.user and dto', async () => {
    const dto = { classId: 'class-1', sessionDate: '2026-06-08' };
    mockSundaySchoolService.createSession.mockResolvedValue({ id: 'session-1' });
  ...
  it('selfMarkPresent — passes req.user and session id', async () => {
    ...
    expect(mockSundaySchoolService.selfMarkPresent).toHaveBeenCalledWith(mockUser, 'session-1');
  });
  ...
  it('bulkMarkAttendance — passes req.user, session id, and dto', async () => {
    ...
    mockSundaySchoolService.bulkMarkAttendance.mockResolvedValue([{ id: 'att-1' }]);
  ...
  it('getSessionRoster — passes req.user and session id', async () => {
    ...
    expect(mockSundaySchoolService.getSessionRoster).toHaveBeenCalledWith(mockUser, 'session-1');
  });
});
