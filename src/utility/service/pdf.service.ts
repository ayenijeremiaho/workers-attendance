...
import {FullEventReport, SessionReport, SessionSlotReport} from '../../service-programme/service/service-session.service';
...
export class PdfService {
    ...
    private drawEventSummaryReport(doc: jsPDF, report: FullEventReport): void {
        ...
        doc.setFont('helvetica', 'bold').setFontSize(9).setTextColor(WHITE).text(this.churchName, MARGIN, 10);
        ...
        doc.setFont('helvetica', 'bold').setFontSize(16).setTextColor(DARK)
            .text(report.eventName.toUpperCase(), PAGE_W / 2, 40, {align: 'center'});
        doc.setFont('helvetica', 'normal').setFontSize(8.5).setTextColor(MUTED)
            .text(`Event Report  ·  ${eventDateFormatted}`, PAGE_W / 2, 49, {align: 'center'});
        const generatedAt = new Date().toLocaleString('en-GB', {
            day: '2-digit', month: '2-digit', year: 'numeric',
            hour: '2-digit', minute: '2-digit', second: '2-digit',
        });
        doc.setFontSize(7.5).text(`Generated: ${generatedAt}`, PAGE_W / 2, 55, {align: 'center'});
        doc.setDrawColor(ACCENT).setLineWidth(0.5).line(MARGIN, 59, PAGE_W - MARGIN, 59);
        ...
        const doneSlots = allRows.filter(({slot}) => slot.status === ServiceSessionSlotStatusEnum.COMPLETED).length;
        const totalAllocSec = allRows.reduce((sum, {slot}) => sum + slot.allocatedMinutes * 60, 0);
        const totalActualSec = allRows.reduce((sum, {slot}) => sum + (slot.actualSeconds ?? 0), 0);
        ...
        const cards: Array<{label: string; value: string; color: string}> = [
            {label: 'SPEAKERS DONE', value: `${doneSlots} / ${totalSlots}`, color: DARK},
            {label: 'TOTAL ALLOCATED', value: this.fmtSecsMM(totalAllocSec), color: DARK},
            {label: 'TOTAL ACTUAL', value: totalActualSec > 0 ? this.fmtSecsMM(totalActualSec) : '—', color: DARK},
            {
        ...
        const tableBody = allRows.map(({serviceName, slot}, idx) => {
            ...
            const actual = slot.actualSeconds != null ? this.fmtSecsMM(slot.actualSeconds) : '—';
            const variance = slot.overrunSeconds != null ? this.fmtOverrunMM(slot.overrunSeconds) : '—';
            const status = this.slotStatusLabel(slot.status, slot.overrunSeconds);
            return [String(idx + 1), speaker, topicSlot, allocated, actual, variance, status];
        });
        ...
        autoTable(doc, {
            ...
            didParseCell: (data) => {
                ...
                if (data.column.index === 6) {
                    ...
                    else if (v === 'Under Time' || v === 'On Time') data.cell.styles.textColor = SUCCESS;
                    else if (v === 'Not Used' || v === 'Pending') data.cell.styles.textColor = MUTED;
                }
            },
        });
    ...
    private slotStatusLabel(status: string, overrunSeconds: number | null): string {
    ...
    private drawProgrammeDraft(doc: jsPDF, programme: ServiceProgramme): void {
        ...
        const dateLabel = event?.eventDate
            ? new Date(event.eventDate).toLocaleDateString('en-GB', {weekday: 'long', day: '2-digit', month: 'long', year: 'numeric'})
            : '—';
        const sorted = [...(programme.slots ?? [])].sort((a, b) => a.position - b.position);

        const y = this.drawOrderOfServiceHeader(doc, slot?.name ?? 'Service Programme', dateLabel);
        this.drawOrderOfServiceTable(doc, y, slot?.name ?? 'Service', slot?.startTime ?? null, sorted);
        this.drawPageFooter(doc);
    }
    ...
    private drawEventProgramme(
        ...
        for (const {slot, programme} of sections) {
            ...
            const sorted = [...(programme.slots ?? [])].sort((a, b) => a.position - b.position);
            y = this.drawOrderOfServiceTable(doc, y, slot.name, slot.startTime ?? null, sorted);
            y += 6;
        }
    ...
    private drawOrderOfServiceHeader(doc: jsPDF, title: string, dateLabel: string): number {
        doc.setFillColor(DARK);
        doc.rect(0, 0, PAGE_W, 22, 'F');
        doc.setFont('helvetica', 'bold').setFontSize(9).setTextColor(WHITE).text(this.churchName, MARGIN, 10);
        doc.setFont('helvetica', 'normal').setFontSize(8).setTextColor(ACCENT).text(this.churchTagline, PAGE_W - MARGIN, 10, {align: 'right'});
        ...
        doc.setDrawColor(ACCENT).setLineWidth(0.5).line(MARGIN, 52, PAGE_W - MARGIN, 52);
        return 58;
    }
    ...
    private drawOrderOfServiceTable(
        ...
        const COL = hasBackup
            ? {sn: 10, events: 48, start: 24, dur: 20, end: 24, minister: 26, backup: 22}
            : {sn: 10, events: 58, start: 26, dur: 22, end: 26, minister: 32, backup: 0};
        ...
        for (let i = 0; i < slots.length; i++) {
            ...
            const rowEnd = cursor ? this.fmtTime(new Date(cursor.getTime() + mins * 60_000)) : '—';
            if (cursor) cursor = new Date(cursor.getTime() + mins * 60_000);
            totalMinutes += mins;

            let minister = '—';
            if (s.member) minister = `${this.cap(s.member.firstname)} ${this.cap(s.member.lastname)}`;
            else if (s.guestName) minister = s.guestName;

            const row: any[] = [String(i + 1), s.topic || (ServiceSlotTypeLabels[s.type] ?? s.type), rowStart, this.fmtDuration(mins), rowEnd, minister];

            if (hasBackup) {
                let backup = '—';
                if (s.backupMember) backup = `${this.cap(s.backupMember.firstname)} ${this.cap(s.backupMember.lastname)}`;
        ...
        const footRow = ['', '', '', totalLabel, finalTime, '', ...(hasBackup ? [''] : [])];
        ...
        const colHeaders = ['S/N', 'EVENTS', 'START TIME', 'DURATION', 'END TIME', 'MINISTER', ...(hasBackup ? ['BACKUP'] : [])];

        autoTable(doc, {
            startY: y,
            margin: {left: MARGIN, right: MARGIN},
            head: [
                [{content: serviceName.toUpperCase(), colSpan: colCount, styles: {halign: 'center', fillColor: DARK, textColor: WHITE, fontStyle: 'bold', fontSize: 10, cellPadding: 4}}],
                colHeaders,
            ],
            body: rows,
            foot: [footRow],
    ...
    private drawNoProgammePlaceholder(doc: jsPDF, y: number, slot: ServiceSlot): number {
        ...
        autoTable(doc, {
            ...
            margin: {left: MARGIN, right: MARGIN},
            head: [
                [{content: slot.name.toUpperCase(), colSpan: 6, styles: {halign: 'center', fillColor: DARK, textColor: WHITE, fontStyle: 'bold', fontSize: 10, cellPadding: 4}}],
            ],
            body: [
                [{content: `${timeLabel ? timeLabel + ' · ' : ''}No programme has been created for this service.`, colSpan: 6, styles: {halign: 'center', textColor: MUTED, fontStyle: 'italic', fontSize: 9, cellPadding: 6}}],
            ],
